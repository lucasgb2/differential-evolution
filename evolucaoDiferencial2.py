...
class DE():
    ...
    def crossoverExp(self, alvo, individuoModificado):
        vetorCrossover = []
        for i in range(alvo.tamanhoCromo):
            ...
            k = random()
            if k <= self.fatorCrossover:
                vetorCrossover.append(individuoModificado.cromossomo[i])
            else:
                ''' Copiando todo o resto do vetor e saindo fora do cruzamento '''
                vetorCrossover = vetorCrossover + alvo.cromossomo[i:]
                break
    ...
    def run(self):
        ...
        for i in range(self.interacoes):
            ...
            for p in range(len(self.populacao)):
                ...
                '''Gerando o experimental com cruzamento Exponencial'''
                individuoExperimental = self.crossoverExp(self.populacao[p], individuoModificado)
        ...
        plt.legend()
        plt.title("Convegência das gerações RAND/1/EXP")
